...
(() => {
  ...
  const fctx = get2DContext(forceCanvas);
  ...
  function state(): ScenarioState {
    if (!latestState) {
      latestState = solveScenario({
        windAngle: +windAngleInput.value,
  ...
  function handleInput() {
    windAngleInput.value = String(
      clampSailableWindAngle(+windSpeedInput.value, +windAngleInput.value, +windAngleInput.value)
    );
  ...
  function arrow(
    ...
    ctx.lineTo(end.x - 11 * Math.cos(screenAngle - 0.5), end.y - 11 * Math.sin(screenAngle - 0.5));
    ctx.lineTo(end.x - 11 * Math.cos(screenAngle + 0.5), end.y - 11 * Math.sin(screenAngle + 0.5));
    ctx.closePath();
    ctx.fill();
    ...
    ctx.fillText(
      label,
      end.x + (u.x >= 0 ? 8 : -ctx.measureText(label).width - 8),
      end.y + (u.y >= 0 ? -8 : 16)
    );
    ctx.restore();
  }
  ...
  function drawBoat(cx: number, cy: number, s: number, helmAngle: number) {
    ...
    ctx.beginPath();
    ctx.moveTo(-23 * s, 75 * s);
    ctx.lineTo(23 * s, 75 * s);
    ctx.stroke();
    ctx.beginPath();
    ...
    ctx.beginPath();
    ctx.moveTo(0, 80 * s);
    ctx.lineTo(rudder.x * 27 * s, 80 * s + rudder.y * 27 * s);
    ctx.stroke();
    ctx.fillStyle = css('--rudder');
    ctx.beginPath();
    ctx.arc(0, 80 * s, 3.5, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }

  function drawForceCard(st: ScenarioState) {
    const w = forceCanvas.clientWidth,
      h = forceCanvas.clientHeight;

    fctx.clearRect(0, 0, w, h);

    const origin = { x: w * 0.5, y: h * 0.59 };
    const radius = Math.min(72, w * 0.22);
    ...
    if (length(trueWindScreen) > 0.01) {
      ...
      if (upwind.x > 0.001) boundaryDistances.push((labelBounds.right - origin.x) / upwind.x);
      if (upwind.x < -0.001) boundaryDistances.push((labelBounds.left - origin.x) / upwind.x);
      if (upwind.y > 0.001) boundaryDistances.push((labelBounds.bottom - origin.y) / upwind.y);
      if (upwind.y < -0.001) boundaryDistances.push((labelBounds.top - origin.y) / upwind.y);

      const availableDistance = Math.min(...boundaryDistances);
      const labelDistance = Math.min(radius + 95, availableDistance * 0.82);
    ...
    fctx.beginPath();
    fctx.arc(origin.x, origin.y, radius, 0, Math.PI * 2);
    fctx.stroke();
    fctx.beginPath();
    fctx.moveTo(origin.x - radius, origin.y);
    fctx.lineTo(origin.x + radius, origin.y);
    fctx.stroke();
    fctx.beginPath();
    fctx.moveTo(origin.x, origin.y - radius);
    fctx.lineTo(origin.x, origin.y + radius);
    fctx.stroke();
    fctx.setLineDash([]);
    fctx.fillStyle = css('--ink');
    fctx.beginPath();
    fctx.arc(origin.x, origin.y, 4, 0, Math.PI * 2);
    fctx.fill();
    fctx.restore();
    ...
    function forceArrow(
      ...
      fctx.lineWidth = options.width ?? 3;
      fctx.setLineDash(options.dashed ? [6, 5] : []);
      fctx.beginPath();
      fctx.moveTo(origin.x, origin.y);
      fctx.lineTo(end.x, end.y);
      fctx.stroke();
      fctx.setLineDash([]);

      fctx.beginPath();
      fctx.moveTo(end.x, end.y);
      fctx.lineTo(end.x - 10 * Math.cos(a - 0.5), end.y - 10 * Math.sin(a - 0.5));
      fctx.lineTo(end.x - 10 * Math.cos(a + 0.5), end.y - 10 * Math.sin(a + 0.5));
      fctx.closePath();
      fctx.fill();
    ...
    forceArrow(st.trueWind, css('--true'), 'true wind', 3.1, radius + 7, {
      alpha: 0.34,
      width: 1.5,
      dashed: true
    });
    forceArrow(st.inducedWind, css('--induced'), 'induced', 5.2, radius, {
      alpha: 0.3,
      width: 1.5,
      dashed: true
    });
    forceArrow(st.apparentWind, css('--apparent'), 'apparent', 3.2, radius + 14, {
      alpha: 0.72,
      width: 2,
      dashed: true
    });
    ...
    const forceMax = Math.max(
      ...
      length(st.sailForce),
      length(st.keelForce),
      length(st.rudderForce),
      length(st.hullForce),
      length(st.totalForce)
    );
  ...
  function drawApparentWindField(w: number, h: number, apparentWind: Vector) {
    ...
    for (let row = 0, y = 72; y < h + spacingY; row++, y += spacingY) {
      ...
      for (let x = 36 - offset; x < w + spacingX; x += spacingX) {
        ...
        ctx.lineTo(endX - 7 * Math.cos(angle - 0.48), endY - 7 * Math.sin(angle - 0.48));
        ctx.lineTo(endX - 7 * Math.cos(angle + 0.48), endY - 7 * Math.sin(angle + 0.48));
        ctx.closePath();
        ctx.fill();
      }
  ...
  function draw() {
    ...
    const cx = w * (w < 600 ? 0.5 : 0.52),
      cy = h * 0.52;
    const scaleBoat = Math.max(0.78, Math.min(1.15, w / 850));

    ctx.strokeStyle = 'rgba(180, 220, 214, .055)';
    ctx.lineWidth = 1;
    const grid = 46;
    ...
    ctx.fillStyle = css('--apparent');
    ctx.beginPath();
    ctx.arc(windOrigin.x, windOrigin.y, 4, 0, Math.PI * 2);
    ctx.fill();
    arrow(windOrigin, st.trueWind, css('--true'), 'true', 115, true, 6, 0.42);
    arrow(windOrigin, st.inducedWind, css('--induced'), 'induced', 90, true, 6, 0.34);
    ...
    const rudderCenter = {
      x: cx + rudderVector.x * 14 * scaleBoat,
      y: cy + (80 + rudderVector.y * 14) * scaleBoat
    };
    ...
    // Keep controls and diagnostics synchronized with the rendered state.
    ...
    $('noGoCaption').textContent =
      st.polar.beatAngle > 0
    ...
    $('helmAngleOut').textContent =
      Math.abs(st.helmAngle) < 0.5
        ? 'Centered'
        : `${Math.abs(st.helmAngle).toFixed(0)}° ${st.helmAngle > 0 ? 'starboard' : 'port'}`;
    $('boatSpeedOut').textContent = st.polar.noGo
      ? '0.00 kn · no-go'
      : `${st.boatSpeed.toFixed(2)} kn`;
    $('apparentMetric').textContent = `${st.awSpeed.toFixed(1)} kn · ${sideLabel(st.apparentFrom)}`;
    $('sailMetric').textContent =
      `${formatForce(length(st.sailForce))} · α ${deg(st.sailAlpha).toFixed(1)}°`;
    $('speedMetric').textContent = st.polar.noGo
      ? `No-go below ${st.polar.beatAngle.toFixed(1)}°`
      : `${st.boatSpeed.toFixed(2)} kn · ${Math.abs(deg(st.leeway)).toFixed(1)}° leeway`;
  ...
  function bindDial(
    ...
    function update(event: PointerEvent) {
      ...
      input.value = String(
        clampSailableWindAngle(+windSpeedInput.value, proposedAngle, +input.value)
      );
      handleInput();
    }
