...
export function solveAnchorGeometry(
  ...
  const chainLength=Math.max(chainLengthInput,depth+.01);
  const suspensionFactor=1.35+windStrength/40*.65;
  const suspendedLength=Math.min(chainLength,Math.max(depth+.01,depth*suspensionFactor));
