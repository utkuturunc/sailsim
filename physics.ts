...
export type TrimOptions = Pick<ComputeOptions, 'windAngle' | 'windSpeed' | 'boatSpeed' | 'yacht' | 'polarEquilibrium'>;
...
if (!POLAR) throw new Error('Dufour polar data must be loaded before the physics module.');
  ...
  const scale = (vector:Vector, factor:number):Vector => ({ x:vector.x * factor, y:vector.y * factor });
  const unit = (vector:Vector):Vector => length(vector) ? scale(vector, 1 / length(vector)) : { x:0, y:0 };
  ...
  function foilCoefficients(alpha:number, config:FoilConfig) {
    ...
    const separation = absAlpha > stall
      ? 1.05 * Math.sin(absAlpha) ** 2 * clamp((absAlpha - stall) / rad(22), 0, 1)
      : 0;
    return { cl, cd:config.cd0 + induced + separation };
  }

  function foilForce(flowVelocity:Vector, chord:Vector, density:number, area:number, config:FoilConfig) {
  ...
  function computeState(options:ComputeOptions) {
    ...
    const sailResult = foilForce(apparentWindMS, chord, AIR_DENSITY, yacht.sailArea, {
      slope:5.4, clMax:1.28, stallDeg:16, cd0:0.035, aspectRatio:4.2, efficiency:0.72
    });
    const sailForce = sailResult.force;
    const boatSpeedMS = boatSpeed * KNOT_TO_MS;
    const keelConfig = { slope:4.7, clMax:1.05, stallDeg:12, cd0:0.012, aspectRatio:2.25, efficiency:0.75 };
    const rudderConfig = { slope:4.9, clMax:1.10, stallDeg:14, cd0:0.014, aspectRatio:2.1, efficiency:0.72 };
    ...
    function hydroAt(leeway:number) {
      if (boatSpeedMS < 0.05) {
        return {
          keelForce:{x:0,y:0}, rudderForce:{x:0,y:0}, hullForce:{x:0,y:0},
          keelAlpha:0, keelCl:0, keelCd:0, rudderAlpha:0, rudderCl:0, rudderCd:0,
          drivingForce:sailForce.y, hullResistance:0, speedRatio:0,
          waterFlow:{x:0,y:0}, lateral:sailForce.x
        };
      }
      ...
      const empiricalResistance = 145 * displacementScale * boatSpeedMS * boatSpeedMS * (1 + 0.55 * speedRatio ** 4);
      ...
      return {
        keelForce:keel.force, rudderForce:rudder.force, hullForce,
        keelAlpha:keel.alpha, keelCl:keel.cl, keelCd:keel.cd,
        rudderAlpha:rudder.alpha, rudderCl:rudder.cl, rudderCd:rudder.cd,
        drivingForce, hullResistance, speedRatio, waterFlow,
    ...
    if (boatSpeedMS >= 0.05) {
      ...
      if (lowResidual * highResidual <= 0) {
        for (let index = 0; index < 34; index++) {
          ...
          else { lo = mid; lowResidual = midResidual; }
        }
        leeway = (lo + hi) / 2;
      } else {
    ...
    return {
      windAngle, windSpeed, sailAngle, helmAngle, boatSpeed, yacht,
      trueWind, inducedWind, apparentWind, apparentWindMS, awSpeed, apparentFrom,
      sailForce, keelForce:hydro.keelForce, rudderForce:hydro.rudderForce,
      hullForce:hydro.hullForce, totalForce,
      sailAlpha:sailResult.alpha, sailCl:sailResult.cl, sailCd:sailResult.cd,
      keelAlpha:hydro.keelAlpha, keelCl:hydro.keelCl, keelCd:hydro.keelCd,
      rudderAlpha:hydro.rudderAlpha, rudderCl:hydro.rudderCl, rudderCd:hydro.rudderCd,
      leeway, yawMoment, boatSpeedMS, hullSpeedMS, speedRatio:hydro.speedRatio,
      drivingForce:hydro.drivingForce, hullResistance:hydro.hullResistance,
      resistanceModel:polarEquilibrium ? 'polar-equilibrium' : 'empirical', waterFlow:hydro.waterFlow,
  ...
  function trimAtSpeed(options:TrimOptions) {
    ...
    function balancedHelm(sailAngle:number) {
      ...
      for (let helm = -24; helm <= 25; helm += 1) {
        ...
        if (!bracketed && previous.result.yawMoment * current.result.yawMoment <= 0) bracketed = { lo:previous, hi:current };
        previous = current;
      }
      if (bracketed) {
        let lo = bracketed.lo;
      ...
      return {
        ...
        result:best.result,
        score:best.result.drivingForce
    ...
    let best = null;
    for (let magnitude = 0; magnitude <= 90; magnitude += 2) {
      const candidate = balancedHelm(tackSign * magnitude);
      if (!best || candidate.score > best.score) best = candidate;
    }
    const coarse = Math.abs(best.sailAngle);
    for (let magnitude = Math.max(0, coarse - 2); magnitude <= Math.min(90, coarse + 2); magnitude += 0.25) {
      const candidate = balancedHelm(tackSign * magnitude);
  ...
  function solveScenario(options:ScenarioOptions) {
    ...
    const trim = trimAtSpeed({
      windAngle, windSpeed, boatSpeed, yacht:options.yacht, polarEquilibrium:boatSpeed > 0
    });
    ...
    return {
      ...
      polar:{
        speed:boatSpeed,
        source:POLAR.source,
        withinChartWindRange:windSpeed >= POLAR.tws[0] && windSpeed <= POLAR.tws[POLAR.tws.length - 1],
  ...
  function debugReport(state:ScenarioState) {
    return {
      ...
      foil:{
        sail:{ alpha_deg:rounded(deg(state.sailAlpha), 3), cl:rounded(state.sailCl), cd:rounded(state.sailCd) },
        keel:{ alpha_deg:rounded(deg(state.keelAlpha), 3), cl:rounded(state.keelCl), cd:rounded(state.keelCd) },
        rudder:{ alpha_deg:rounded(deg(state.rudderAlpha), 3), cl:rounded(state.rudderCl), cd:rounded(state.rudderCd) }
      },
      force_N:{
...
const api = Object.freeze({
    KNOT_TO_MS, AIR_DENSITY, WATER_DENSITY, DEFAULT_YACHT, POLAR,
    rad, deg, length, add, scale, unit, dot, cross, clamp,
    normalizeSignedAngle, polarMetadata, polarSpeed, clampSailableWindAngle,
    signedAoA, foilCoefficients, foilForce,
    computeState, trimAtSpeed, solveScenario, debugReport
  });
